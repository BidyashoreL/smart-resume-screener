...
async def upload_resume(
    ...
    reused across multiple future screenings without re-processing.
    """
    ...
    logger.info("Processed resume upload -> candidate_id=%s", candidate.id)

    return ResumeUploadResponse(candidate_id=candidate.id, status="processed", name=candidate.name)


@router.get("/{candidate_id}", response_model=CandidateResponse)
def get_resume(candidate_id: str, db: Session = Depends(get_db)) -> CandidateResponse:
...
def list_resumes(db: Session = Depends(get_db)) -> list[CandidateResponse]:
